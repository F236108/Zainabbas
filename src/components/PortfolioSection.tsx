...
const PortfolioSection = () => {
  ...
  return <section id="portfolio" className="py-20 bg-background">
      <motion.div className="section-container" initial="hidden" whileInView="visible" viewport={{
      once: true,
      amount: 0.1
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {projects.map(project => <motion.div key={project.id} variants={cardVariants}>
              <Card className="bg-secondary overflow-hidden border-electric/10 transform transition-all duration-500 hover:shadow-xl hover:shadow-electric/20 hover:translate-y-[-5px] h-full flex flex-col">
                <div className="h-48 overflow-hidden object-cover">
                  <img src={project.image} alt={project.title} className="w-full h-full object-cover transition-all duration-700 hover:scale-110" />
                </div>
                ...
                <CardContent className="flex-grow">
                  ...
                  <div className="flex flex-wrap gap-2 mt-4">
                    {project.tags.map((tag, index) => <span key={index} className="text-xs bg-muted px-2 py-1 rounded-full text-gray-300 transition-all duration-300 hover:bg-electric/20 hover:text-electric">
                        {tag}
                      </span>)}
                  </div>
                </CardContent>
                <CardFooter className="flex justify-between">
                  <Button variant="ghost" className="text-electric hover:bg-electric/10 hover:text-electric group transition-all duration-300">
                    <span>View Details</span>
                    <ExternalLink className="ml-2 h-4 w-4 transform transition-all duration-300 group-hover:translate-x-1" />
                  </Button>
                  <Button variant="outline" className="text-electric border-electric hover:bg-electric/10 group transition-all duration-300" onClick={() => window.open('https://github.com/F236108', '_blank')}>
